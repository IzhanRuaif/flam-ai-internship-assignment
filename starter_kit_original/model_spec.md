...
Each row of `bench_log.csv` is one run: `num_requests` identical requests submitted simultaneously with the given `prompt_len` and `gen_len` (all requests generate exactly `gen_len` tokens, no early stopping).
